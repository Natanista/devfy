...
class ProjetoAdapterAberto(val context: Context, val projetoList: List<Projeto>): RecyclerView.Adapter<ProjetoAdapterAberto.ProjetoHolder>() {
    ...
    override fun onBindViewHolder(holder: ProjetoHolder, position: Int) {
        var projetosAbertos = listOf<Projeto>()
        projetoList.forEach{
            if (it.status == "Aberto"){
                projetosAbertos.toMutableList().add(it)
            }
        }
        val currentItem = projetosAbertos[position]
